...
public class MediaMetadataService :
    IRequestHandler<MakePhotoThumbnailRequest, byte[]?>,
    IRequestHandler<MakeVideoThumbnailRequest, byte[]?>,
    IRequestHandler<ReadVideoMetadataRequest, VideoMetadata?>
{
    ...
    {
        return Task.FromResult(request.filePath.MakePhotoThumbnail(request.DoubleSize, request.Orientation));
    }
    ...
    public async Task<byte[]?> Handle(MakeVideoThumbnailRequest request, CancellationToken cancellationToken)
    {
        ...
        await $"ffmpeg -i {request.filePath} -ss 00:00:00.010 -vframes 1 {videoThumbnailFilePath}".Bash(_logger);
    ...
    public async Task<VideoMetadata?> Handle(ReadVideoMetadataRequest request, CancellationToken cancellationToken)
    {
        ...
        await $"ffprobe -v error -select_streams v:0 -show_entries stream=height,width -show_format {request.FilePath} > {videoMetadataFilePath} 2>&1"
            .Bash(_logger);
